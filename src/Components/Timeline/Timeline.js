import React from 'react';
import { Label } from '../../SubComponents/Label';
import { ImgCard } from '../../SubComponents/ImgCard';
import { UnorderedList } from '../../SubComponents/UnorderedList';
import './Timeline.css';
const eighties = 'Born in Denver, CO to parents Tom and Lynette.';
const ohseven = 'Enlisted on six year journey that took me through two tours in Iraq as an Army Combat Medic';
const thirteen = 'Studied Marketing at Texas A&M, graduating Magna Cum Laude in less than three years.';
const sixteen = 'Scored a job in marketing, but yearned to work in technology.';
const seventeen = 'Made the life-changing decision to attend Galvanize Web Development Immersive';
const eighteen = 'Developing technology, directing events, teaching JavaScript, and enjoying life to the fullest.';
const timelineArray = [
    {"1980's": ["https://s3-us-west-2.amazonaws.com/mgx-photos/little-chris.jpg", eighties]},
    {"2007": ["https://s3-us-west-2.amazonaws.com/mgx-photos/syrian-border.jpg", ohseven]},
    {"2013": ["https://s3-us-west-2.amazonaws.com/mgx-photos/Chris+A%26M.jpg", thirteen]},
    {"2016": ["https://s.thestreet.com/files/tsc/v2008/photos/contrib/uploads/scientificgames_600x400.jpg", sixteen]},
    {"2017": ["data:image/png;base64,", seventeen]},
    {"2018": ["https://s3-us-west-2.amazonaws.com/mgx-photos/chris-2018.jpg", eighteen]},
];
const timeline = timelineArray.map(element => {
    const year = Object.keys(element);
    const imgUrl = Object.values(element)[0][0];
    const content = Object.values(element)[0][1];
    if (timelineArray.indexOf(element) % 2 === 0) {
        return (
            <div key={year} className="Margin-bottom Left" id="Timeline-line-item-div">
                <Label labelClassName="Label markerFont" labelId="Likes-date-label" text={year} />
                <ImgCard 
                containerClassName="Card-container-shadow"
                containerId="Timeline-card-container"
                imgClassName="Card-container-inset Image"
                imgId="Timeline-card-img"
                src={imgUrl}
                cardContent={content}
                cardContentId="Timeline-card-content"
                cardContentClass="nanumFont"
                />
            </div>
        );
    } else {
        return (
            <div key={year} className="Margin-bottom Right" id="Timeline-line-item-div">
                <Label labelClassName="Label markerFont" labelId="Likes-date-label" text={year} />
                <ImgCard 
                containerClassName="Card-container-shadow"
                containerId="Timeline-card-container"
                imgClassName="Card-container-inset Image"
                imgId="Timeline-card-img"
                src={imgUrl}
                cardContent={content}
                cardContentId="Timeline-card-content"
                cardContentClass="nanumFont"
                />
            </div>
        );
    }
});


export const Timeline = props => {
    return(
        <div className={props.className} id="Timeline-div">
            {/* <div id="Border-container">
                <div id="Left"></div>
                <div id="Right"></div>
            </div> */}
            <UnorderedList ulId="Timeline-list" content={timeline} />
        </div>
    );
}